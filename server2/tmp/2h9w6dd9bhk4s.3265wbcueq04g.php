<nav class="filters-form daterange <?= (isset($hidden) && $hidden ? 'is-hidden' : '') ?>">
    <?php $USE_RANGE    = false;
        $USE_INTERVAL = false;
    ...
    <?php if ($USE_INTERVAL): ?>
            ...
            <div class="control has-icons-left">
                ...
                <span class="icon">
                    <?php echo $this->render('images/icons/stopwatch.svg',NULL,get_defined_vars(),0); ?>
                </span>

                <input type="hidden" name="date_from" />
                <input type="hidden" name="date_to" />
                <input type="hidden" name="offset" value="<?= ($clock_offset) ?>"/>
            </div>
    ...
    <?php if ($USE_RANGE): ?>
        <input type="hidden" name="date_from" />
        <input type="hidden" name="date_to" />
        <input type="hidden" name="offset" value="<?= ($clock_offset ?? 0) ?>"/>

        <div class="field">
            <p class="control has-icons-left">
                <input type="datetime-local" class="input" name="date_from_local" step="1">
                <span class="icon">
                    <?php echo $this->render('images/icons/calendar.svg',NULL,get_defined_vars(),0); ?>
                </span>
            </p>
        </div><div
            class="field">
